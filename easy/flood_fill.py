...
# Runtime: 76 ms, faster than 76.78% of Python3 online submissions for Flood Fill.
# Memory Usage: 12.9 MB, less than 100.00% of Python3 online submissions for Flood Fill.
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        ...
        while queue:
            ...
            if image[row][col] == original_color:
                ...
                for n_r, n_c in [(row + 1, col), (row, col + 1), (row, col - 1), (row - 1, col)]:
                    if n_r >= 0 and n_r < len(image) and n_c >= 0 and n_c < len(image[0]):
