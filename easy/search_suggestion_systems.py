...
# Runtime: 104 ms, faster than 61.93% of Python3 online submissions for Search Suggestions System.
# Memory Usage: 15.2 MB, less than 100.00% of Python3 online submissions for Search Suggestions System.
class Solution:
    def suggestedProducts(self, products: List[str], searchWord: str) -> List[List[str]]:
        ...
        for i in range(len(searchWord)):
            result = []
            new_products = []
            short_word = searchWord[:i + 1]
            for p in products:
                if len(p) < i:
                    continue
                elif p[:i + 1] == short_word:
                    result.append(p)
